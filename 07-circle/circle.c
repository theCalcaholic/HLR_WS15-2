...
void
print_rand_array(char* text, int rank,int number,int store,int size,int* buf)
{
  ...
  if(rank == 0)
  {
	...
	for(int k = 0; buf[k] < 25 ;k++)
	{
  	...
  	{
	  int randmessage[store+1];
	  MPI_Recv (randmessage,number+1,MPI_INT,i,100,MPI_COMM_WORLD,NULL);
  ...
  {
	MPI_Send(buf, number+1, MPI_INT, 0, 100, MPI_COMM_WORLD);	
  }
}

int*
init (int number,int store, int rank)
{
  //todo
  int* buf = malloc(sizeof(int)* (store + 1));		//Es wird Speicher für die neuen Zufallszahlen reserviert.
...
circle (int* buf,int rank,int predecessor, int successor, int size, int store)
{
  ...
  if(rank == 0)
  {
	MPI_Send(&newbuf[0], 1, MPI_INT, predecessor, 11, MPI_COMM_WORLD);		//Senden der speziellen Zahl
  }
...
main (int argc, char** argv)
{
  ...
  print_rand_array("Before",rank,number_of_rand,number_of_rand_max,size,buf);
  buf = circle(buf,rank,predecessor,successor,size, number_of_rand_max);
  print_rand_array("After",rank,number_of_rand,number_of_rand_max,size,buf);
  MPI_Finalize();
  return EXIT_SUCCESS;
}
